...
def simulate_episode(ddir, odir):
        ...
        recenter_center = np.array([0., 0., 0.])
        ...
        y_scale = 1
        z_scale = 1
        left_xyz = np.array([0, 0, 0])
        right_xyz = np.array([0, 0, 0])
        episode_length = 100

        data_dir = "./output_data/"
        json_data = sorted(os.listdir(data_dir))
        index = 0
        recenter_center = np.array([0., 0.4, 0.])
        time_slice = episode_length/len(json_data)
        time_cal = 0
        place_obj = False
        cube_pose = np.array([0., 0., 0.])
        cube_quat = np.array([1., 0., 0., 0.])

        # hand_directory = ddir + "/hand_pose/"
        # obj_directory = ddir + "/obj_pose/"

        input_filenames = sorted(os.listdir(ddir))
        ...
        for i in range(len(input_filenames)):  
            ...
            #         continue
            with open(os.path.join(ddir, input_filenames[i]), 'r') as json_file:
                json_data = json.load(json_file)
            # print(hand_points.shape)

            hand_points = np.concatenate((json_data['displacements'][0]['hand_skelotons'], json_data['displacements'][1]['hand_skelotons']), axis=0)
            # print(hand_points.shape)

            # obj_dir = "./obj_pose/0000" + str(i) + ".txt"
            box_points = np.array(json_data['object']['corners'])
            ...
            reverse = False
            if np.mean(hand_points[0:21, 0]) > np.mean(hand_points[21:, 0]):
                reverse = True
            ...
            if reverse:
                left_xyz = (np.array([np.mean(hand_points[left_finger, 0]), np.mean(hand_points[left_finger, 1]), np.mean(hand_points[left_finger, 2])]) + recenter_center).tolist()
                right_xyz = (np.array([np.mean(hand_points[right_finger, 0]), np.mean(hand_points[right_finger, 1]), np.mean(hand_points[right_finger, 2])]) + recenter_center).tolist()
                # get the plane of the hand
                hand_plane_vec = np.cross(hand_points[4]-hand_points[3], hand_points[12]-hand_points[11])
                hand_plane_vec = hand_plane_vec * 0.1 / np.linalg.norm(hand_plane_vec) + hand_points[0]
                hand_plane_vec_r = np.cross(hand_points[33]-hand_points[32], hand_points[25]-hand_points[24])
                ...
                right_xyz = (np.array([np.mean(hand_points[left_finger, 0]), np.mean(hand_points[left_finger, 1]), np.mean(hand_points[left_finger, 2])]) + recenter_center).tolist()
                left_xyz = (np.array([np.mean(hand_points[right_finger, 0]), np.mean(hand_points[right_finger, 1]), np.mean(hand_points[right_finger, 2])]) + recenter_center).tolist()
                # get the plane of the hand
                hand_plane_vec_r = np.cross(hand_points[4]-hand_points[3], hand_points[12]-hand_points[11])
                hand_plane_vec_r = hand_plane_vec_r * 0.1 / np.linalg.norm(hand_plane_vec_r) + hand_points[0]
                hand_plane_vec = np.cross(hand_points[33]-hand_points[32], hand_points[25]-hand_points[24])
                hand_plane_vec = hand_plane_vec * 0.1 / np.linalg.norm(hand_plane_vec) + hand_points[21+0]

                right_dir = np.array([np.mean(hand_points[left_third_joint, 0]), np.mean(hand_points[left_third_joint, 1]), np.mean(hand_points[left_third_joint, 2])]) - hand_points[0]
                left_dir = np.array([np.mean(hand_points[right_third_joint, 0]), np.mean(hand_points[right_third_joint, 1]), np.mean(hand_points[right_third_joint, 2])]) - hand_points[21+0]
                right_dir = right_dir - np.dot(right_dir, np.array([0,0,1])) * np.array([0,0,1])
                right_dir = right_dir / np.linalg.norm(right_dir)
                left_dir = left_dir - np.dot(left_dir, np.array([0,0,1])) * np.array([0,0,1])
                left_dir = left_dir / np.linalg.norm(left_dir)
            ...
            print(left_r_axis, left_r_angle, left_p_axis, left_p_angle)
            try:
            ...
            if if_close(np.mean(hand_points[right_finger,:], axis=0)-box_points[0], box_x, box_y, box_z):
                if not reverse:
                    left_hand_open = 0
                    if not place_obj:
                        place_obj = True
                        cube_pose = left_xyz
                        cube_quat = Quaternion(axis=left_r_axis, degrees=left_r_angle).elements
                else:
                    right_hand_open = 0
                    if not place_obj:
                        place_obj = True
                        cube_pose = right_xyz
                        cube_quat = Quaternion(axis=right_r_axis, degrees=right_r_angle).elements
                ...
                if not reverse:
                    ...
                    right_hand_open = 1
            if if_close(np.mean(hand_points[left_finger,:], axis=0)-box_points[0], box_x, box_y, box_z):
                if not reverse:
                    right_hand_open = 0
                    if not place_obj:
                        place_obj = True
                    ...
                    if not place_obj:
                        place_obj = True
                        cube_pose = left_xyz
                        cube_quat = Quaternion(axis=left_r_axis, degrees=left_r_angle)
            else:
                if not reverse:
                    right_hand_open = 1
                else:
            ...
            left_trajectory.append({"t": time_cal, 
                                    "xyz": left_xyz, 
                                    "quat": left_quat.elements.tolist(), 
                                    "gripper": left_hand_open})
            right_trajectory.append({"t": time_cal, 
                                    "xyz": right_xyz, 
            ...
            # ax. set_ylim3d(-1, 1)
            # ax.set_zlim3d(-1, 1)
            ax.scatter(hand_points[ :, 0], hand_points[ :, 1], hand_points[ :, 2], s=50, color=hand_color)
            ax.scatter(box_points[ :, 0], box_points[ :, 1], box_points[ :, 2], s=50, color = color_array)
            # print(np.min(box_points[:, 2]), np.max(box_points[:, 2]))
            # ax.scatter(hand_plane_vec[0], hand_plane_vec[1], hand_plane_vec[2], s=50, color = [0, 0, 0])
            # ax.scatter(hand_plane_vec_r[0], hand_plane_vec_r[1], hand_plane_vec_r[2], s=50, color = [0, 0, 0])
            # ax.plot([hand_points[0, 0], hand_plane_vec[0]], [hand_points[0, 1], hand_plane_vec[1]], [hand_points[0, 2], hand_plane_vec[2]], 'k')
            # ax.plot([hand_points[21, 0], hand_plane_vec_r[0]], [hand_points[21, 1], hand_plane_vec_r[1]], [hand_points[21, 2], hand_plane_vec_r[2]], 'k')
            for _c in hand_connection:
                ax.plot ( [hand_points[ _c[0], 0], hand_points[ _c[1], 0]],
            ...
            for _c in obj_connection:
                ax.plot ( [box_points[ _c[0], 0], box_points[ _c[1], 0]],
        ...
        render_cam_name = 'top' # top angle vis
        print("time_cal: ", time_cal)

        print("cube_pose: ", cube_pose, cube_quat)
        cube_pose[2] = 0.01
        ...
        # new_size = np.array([box_x])  # New half-dimensions
        # print(physics.model.geom_size[box_geom_id])
        task = BimanualViperXEETask(cube_pose=cube_pose, cube_quat=cube_quat.tolist())
        ...
        for eps_idx in range(num_episodes):
            for step in range(episode_length-10):
                action = policy(ts)
        ...
        physics = mujoco.Physics.from_xml_path(xml_path)
        task = TransferCubeTask(cube_pose=cube_pose, cube_quat=cube_quat.tolist())
        env = control.Environment(physics, task, time_limit=20, control_timestep=DT,
                                        n_sub_steps=None, flat_observation=False)
        # box_geom_id = physics.model.name2id('red_box', 'geom')
        ...
        if replay_reward >= 20:
            ...
            dic_path = os.path.join(odir, f'transfercup')
            num_data = len(os.listdir(dic_path))
            dataset_path = os.path.join(dic_path, f'transfercup_{num_data}')
            t0 = time.time()
